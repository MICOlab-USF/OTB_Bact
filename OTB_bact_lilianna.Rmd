...
ggplot(bact_all, aes(x = Station, y = Mean_Conc)) +
  ...
  labs(
    title = "Bacterial Concentration (Gate R2) Across Sampling Dates",
    x = "Station",
    y = "Mean Concentration (cells/μL)"
  ) +
...
```{r}
basemap(limits = limits) +
  geom_point(data = bact_geo,
             aes(x = Long, y = Lat, color = Station, size = Mean_Conc),
             alpha = 0.8) +
  scale_color_viridis_d(option = "turbo") +
  scale_size_continuous(name = "Mean Concentration (cells/μL)") +
  labs(title = "Bacterial Concentration at OTB Stations",
